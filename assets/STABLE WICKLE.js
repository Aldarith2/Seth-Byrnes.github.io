(function () {
  ...
  function setupDividerWickle() {
    ...
    function say(lines) {
      ...
      if (!sequence.length) sequence = ['Oh. You found Wickle.'];
    ...
    function chooseDifferentLine(builder, fallback) {
      var first = normalizeLine(builder());
      var second = normalizeLine(builder());
      if (second === first) second = normalizeLine(fallback || builder());
      if (second === first && fallback) second = normalizeLine(fallback);
      return [first, second];
    }
    ...
    function getContextDialogue(host, clicked, forceTrailer) {
      ...
      }, chooseFreshLine((WICKLE_DIALOGUE.global[getPageRegion()] || {}).normal || ['Wickle was inspecting this bit.']));
    }

    function getSwitchDialogue(kind) {
      return chooseDifferentLine(function () {
        return getSwitchLine(kind);
      }, chooseFreshLine(WICKLE_DIALOGUE.global.switch.idle || ['Too bright.']));
    }
    ...
    function showWickle(hostTypeOverride) {
      ...
      if (state.resurrectLinePending && currentPageKey === 'index.html' && state.hostType !== 'switch') {
        ...
        }, chooseFreshLine((WICKLE_DIALOGUE.global.top || {}).weird || ['Wickle returned with notes.'])));
        state.resurrectLinePending = false;
        sessionStorage.removeItem(resurrectStorageKey);
        state.bubbleTimer = window.setTimeout(function () {
          hideWickle(true);
        }, randomRange(5200, 7200));
        return;
      }
